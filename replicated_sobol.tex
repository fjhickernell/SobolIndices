...
An improvement to reduces this cost lies in the use of replicated designs. The notion of replicated designs was first introduced by McKay through the introduction of replicated Latin Hypercubes samples. Here, we define replicated designs in a wider framework:
...
\end{equation}
where $x_{i,j} = \sum_{k \geq 1}x_{i,j,k}b^{-k}$ is the radical inverse $b$-ary decomposition of point $x_{i,j}$ and $i = \sum_{k=0}^{m-1}i_kb^{k}$ is the $b$-ary decomposition of $i$. For $m' < m$ the matrices $C_j^{m'}$ are embedded in the upper left corners of matrices $C_j^{m}$. Thus, the digital nets possess an innate nested structure resulting from this embedding. That is $\mathcal{P}_{m'} \subset \mathcal{P}_m \subset \dots \subset \mathcal{P}_{\infty}$. 
