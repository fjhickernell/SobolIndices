...
In this section we introduce a particular type of quasi-Monte Carlo points to estimate $\eqref{first_order_indice}$, which will allow us to evaluate the integrand at points $\{\left(\vx_i,{\vx'}_{i,-u}\right)\}_{i=0}^{2^m}$ only one time to estimate all $d$ indices.
...
\begin{defin}
A digital sequence $\{{\vx'}_i\}_{i\in\mathbb{N}_0}$ is the replicated digital sequence of $\{\vx_i\}_{i\in\mathbb{N}_0}$ if for all $m\geq 0$, $\{{\vx'}_i\}_{i=0}^{b^m-1}$ is a replicated design of $\{{\vx}_i\}_{i=0}^{b^m-1}$.
\end{defin}
...
Considering the generating matrices to be upper triangular and without zeros on the diagonal, one ensures that $x_{i,j,k}=0$, for $m<k$ and $0 \leq i \leq b^m-1$. Since the block $C_j^{m\times m}$ is full rank, each $x_{i,j}\neq x_{r,j}$ when $i\neq j$. Therefore, excluding the case ${\vx}_i={\vx'}_i$ for all $i\in\mathbb{N}_0$, it is enough to choose $C_j\neq C_j'$ for at least $d-1$ dimension. When $b=2$, this is the case of Sobol' sequences and one could simply consider $C_{j+d},\dots,C_{2d}$ to generate the replicated sequence.
