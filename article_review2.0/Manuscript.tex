...
\end{equation}
\comment{Equation \eqref{mu.est} can be improved if we use the $2n$ evaluations $f(\vx_i)$ and $f(\vx'_i)$. While this would be an important improvement to estimate the indices' denominator, the error construction described in Section \ref{sec:3} would require additional computations. Because of the new computations in the algorithm, the additional benefit of using $2n$ evaluations could become more costly than beneficial.}
