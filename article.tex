...
As shown in \cite[Sec. 3]{HicJim}, the group structure of the digital nets guarantees the property below affecting any Walsh basis $\phi_{\vk}(\vx):=\e^{2 \pi \sqrt{-1} \ip{\vk}{\vx}/b}$,
...
\end{align*}
If $\vI:=(I_1,I_2,I_3,I_4)$, we can define $\underline{S}_u(\vI)$ and $\overline{S}_u(\vI)$ as functions over a vector of integrals $\vI$. If we estimate $\vI$ by $\widehat{\vI}$ and error bound $\varepsilon_{\vI}$ according to \ref{section.error.algo}, then $\vI\in B_{\varepsilon_{\vI}}(\widehat{\vI}):=[\widehat{\vI}-\varepsilon_{\vI},\widehat{\vI}+\varepsilon_{\vI} ]$.

Alternatively to the common estimators $\widehat{\underline{S}_u}=\underline{S}_u(\widehat{\vI})$ and $\widehat{\overline{S}_u}=\overline{S}_u(\widehat{\vI})$, we propose the following estimators:
\begin{align*}
...
\end{align*}
These new estimators allow us to say that $\underline{S}_u\in [\widehat{\underline{S}_u} - \varepsilon_{\underline{S}_u}, \widehat{\underline{S}_u} + \varepsilon_{\underline{S}_u}]$ and $\overline{S}_u\in [\widehat{\overline{S}_u} - \varepsilon_{\overline{S}_u}, \widehat{\overline{S}_u} + \varepsilon_{\overline{S}_u}]$.

For a better understanding of these estimators, we provide and example in dimension 1 below,
\begin{figure}[H]
\centering
