...
where: 
\begin{itemize}
\item[$\bullet$] $2^{m_u}$ is the number of evaluations $f\left(\vx_{i,u}:{\vx'}_{i,-u}\right)$ used to estimate both $\underline{S}_u$ and $\overline{S}_u$.
...
\end{itemize}
This cost can be seen as a generalisation of the one specified in Theorem \ref{saltelli.theorem}. If all $m_u$ are equal, the cost of Variant $A$ becomes $2^{m^\star}(d+2)$ and we recover the cost of Theorem \ref{saltelli.theorem} with $n=2^{m^\star}$.
...
\end{equation}
where: \begin{itemize}
\item[$\bullet$] for $j\in \Gamma$, $2^{m''_{u_j}}$ is the number of evaluations $f\left(\vz_{i,u_j}:\vx_{i,-u_j}\right)$ to estimate small first-order indices,
\item[$\bullet$] for $j\in \Gamma$, $2^{m'_{u_j}}$ is the number of evaluations $f\left(\vx_{i,j}:{\vx'}_{i,-j}\right)$ to estimate both small first-order and total effect indices,
\item[$\bullet$] for $j\in \mathcal{D}/\Gamma$, $ 2^{m_{u_j}}$ is the number of evaluations $f\left(\vx_{i,j}:{\vx'}_{i,-j}\right)$ to estimate non small first-order indices,
\item[$\bullet$] $2 \times 2^{m^{\star}}$ is the number of evaluations $f\left(\vx_{i}\right)$ and $f\left({\vx'}_{i}\right)$ used in the estimation of each first-order and total effect index.
\end{itemize}
...
\begin{itemize}
\item[1)] for $j \in \Gamma$, the total effect index $\ov{$S$}_j$ requires as much or more evaluations $f\left(\vx_{i,u_j}:{\vx'}_{i,-u_j}\right)$ than the first-order index $\underline{S}_j$. Since the total effect estimator is the same, as a consequence we have $m'_{u_j}=m_{u_j}$ and $c_j >0$.
\item[2)] for $j \in \Gamma$, the total effect index $\ov{$S$}_j$ requires less evaluations $f\left(\vx_{i,u_j}:{\vx'}_{i,-u_j}\right)$ than the first-order index $\underline{S}_j$. In this case, if both $m''_{u_j} < m_{u_j}$ and $m'_{u_j} < m_{u_j}$ then $c_j \leq 0$.  
\end{itemize}
We can write $\Gamma = \Gamma_1 \cup \Gamma_2$, $\Gamma_1 \cap \Gamma_2=\varnothing$, to separate the number of times case $1)$ or case $2)$ are verified. We expect to have more case $2)$ than case $1)$. The reason is that the numerator of the first-order index (\ref{}) requires to estimate $2d-1$ integrals against only $d+1$ integrals for the numerator of the total effect index (\ref{}). Thus, it seems reasonable to suppose that it takes less points to evaluate (\ref{}) than (\ref{}). 
...
In pratice, one does not know which are the small Sobol' indices. To overcome this issue, we propose the following alternative for Variant $A$. Assume that there exists one or multiple  $u \in \mathcal{D}$ such that estimates $\widehat{\underline{S}_u}^{(m)}$ are smaller than a specified threshold at the end of the first iteration of Algorithm \ref{recursive.algorithm}. Then, for these indices, at the next iteration estimator (\ref{first.order.est}) is switched for (\ref{correlation2}) and a third Sobol' sequence $\mathcal{P''}_{m}=\{\vz_i\}_{i=0}^{2^m-1}$ is constructed for the evaluations $f({\vz}_{i,u}:{\vx}_{i,-u})$.
