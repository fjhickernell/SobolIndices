...
Let $f$ represents a deterministic numerical model on $[0,1)^d$, $d \geq 1$. Sensitivity measures known as Sobol' indices are used to assess which inputs of $f$ are influential on the output. These indices are scalar between $0$ and $1$ whose values are interpreted as follows: the closer to $1$ the more influential the index. Among all Sobol' indices one can distinguish first-order and total effect indices. The former measure the effect of an input, the latter measure the effect of an input and its interactions.

When dealing with complex numerical models, analytical expressions of Sobol' indices are often inaccessible. In such cases, one must rely on an estimation of these indices. The original estimation procedure is due to Sobol' \cite{Sobol'}. Later on, Saltelli \cite{Saltelli} proposed strategies to estimate sets of Sobol' indices in one go through the use of a combinatorial formalism. While elegant, these strategies require a large number of model evaluations.  A cost efficient alternative was introduced in \cite{Mara} to estimate first-order indices. This alternative called replication procedure has recently been further studied  in \cite{Tissot} and generalized to the estimation of closed second-order indices.

A practical problem concerning the use of these methods is how large to chose the number of model evaluations to ensure that Sobol' estimates are accurate enough. This paper addresses this challenge by proposing a reliable error bound for Sobol' indices based on digital sequences. The error bound is constructed based on the discrete Walsh decomposition of the Sobol' indices integrals. We propose a sequential estimation procedure of Sobol' indices using the error bound as stopping criterion. The procedure operates under the assumption that all integrands satisfy a cone condition (see \cite{HicJim}).

The next section introduces Sobol' indices and reviews both Saltelli's strategy to estimate first-order and total effect Sobol' indices and the replication procedure. Our main contribution is detailed in  Section \ref{sec:3}. We first review the construction of the error bound for the estimation of an integral then generalize it for Sobol' indices. Section \ref{sec:4} is devoted to our sequential estimation procedure in the form of an algorithm. It combines the error bound of Section \ref{sec:3} and one of the two estimation procedure of Section \ref{sec:2}. We also discuss a potential improvement to estimate small first-order indices. Illustrations of our procedure are presented in Section \ref{sec:5}. 
...
Denote by $\vx=(x_1,\dots,x_d)$ the vector of inputs of $f$ and $\mathcal{D}=\{1,\dots,d\}$. %The uncertainty on $\vx$ is modeled by a random vector that we suppose uniformly distributed on $\cube$.
...
We assume $f \in \mathcal{L}^2[0,1)^d$. Denote by $\mu$ and $\sigma^2$ the mean and variance of $f$.
...
\end{align}
while the variance of $f$ is evaluated by:
\begin{equation}
...
\label{sec:2.3}
The aim of this paper is to propose a sequential procedure to estimate first-order and total effect Sobol' indices. Generally, in most sequential procedures that estimate Sobol' indices, the stopping criterion is a quantity of interest built directly upon the estimates. Such stopping criteria often involve hyper-parameters hard to tweak but above all fail to guarantee any error bound on the estimates. 

Our sequential procedure stands apart from others with the construction of a robust stopping criterion. This criterion is an error bound based on the discrete Walsh decomposition of integrals (\ref{first.order}), (\ref{total.effect}) and (\ref{eq.sigma.mu}). This decomposition use the close set property of digital sequences. As such, our procedure relies on an iterative construction of Sobol' sequences. This construction is performed accordingly to the multiplicative approach presented in \cite{GJAHMP}.

The construction of the error bound is the subject of the next section. Our sequential procedure is detailed in section \ref{sec:4}.


\section{Reliable error bound for Sobol' indices}
\label{sec:3} 
We start by reviewing the construction of the error bound proposed in \cite{HicJim} for the estimation of a $d$-dimensional integral. Then, we present an extension of this error bound for Sobol' indices. This extension exploits the integral formula of a Sobol' index.
...
\begin{equation}
\absolute{I - \frac{1}{b^m}\sum_{\vx\in\mathcal{P}_m}f(\vx)}
\le \frac{\tS_{\ell,m}(f)\homega(m) \wcomega(m-\ell)}{1 - \homega(m-\ell) \wcomega(m-\ell)} = \epsilon_I, \label{errbd}
\end{equation}
where one may increase $m$ until the error bound $\epsilon_I$ is small enough.
...
\end{align*}
where $\vI=(I_1,I_2,I_3,I_4)$. $\underline{S}_u(\vI)$ and $\overline{S}_u(\vI)$ are defined as functions over the vector of integrals $\vI$. If we estimate $\vI$ by $\widehat{\vI}$ with the vector of error bounds $\varepsilon_{\vI}=(\epsilon_{I_1},\epsilon_{I_2},\epsilon_{I_3},\epsilon_{I_4})$ according to \ref{sec:3.1}, we obtain $\vI\in B_{\varepsilon_{\vI}}(\widehat{\vI})=[\widehat{\vI}-\varepsilon_{\vI},\widehat{\vI}+\varepsilon_{\vI} ]$. 

Then, alternatively to the common Sobol' indices estimators (\ref{common.sobol.est}), we can define the two following estimators with the respective error bounds:
\begin{gather}
\begin{aligned}
...
\end{gather}
Maximizing $\underline{S}_u(\vI)$ (resp. $\overline{S}_u(\vI)$) is done through maximizing the numerator $I_1$ (resp. $I_2$) and minimizing the denominator $I_3-I_4^2$. At the opposite, minimizing $\underline{S}_u(\vI)$ (resp. $\overline{S}_u(\vI)$) is done through minimizing the numerator $I_1$ (resp. $I_2$) and maximizing the denominator $I_3-I_4^2$. %As an example, Figure \ref{fig:1} below illustrate the maximization process for $\vI$ unidimensional.
%\begin{figure*}
%\centering
...
\label{sec:4}
The sequential estimation procedure we propose combine the error bounds $\varepsilon_{\underline{S}_u}$ and $\varepsilon_{\overline{S}_u}$ presented in the previous section with one of the two estimation strategies of section \ref{sec:2.2}: Saltelli's strategy and the replication procedure. 

We start be detailing our procedure in the form of an algorithm. Then, we discuss a possible improvement by considering a new estimator recently introduced in \cite{Owen} for the estimation of first-order indices.

\subsection{Sequential algorithm and its cost}
\label{sec:4.1}

Algorithm \ref{recursive.algorithm} summarizes the main steps of our sequential procedure. First, one must fix the tolerance $\epsilon >0$ for the Sobol' estimates. Then, we set $m=m_0$ and construct the two designs $\mathcal{P}_{m}=\{\vx_i\}_{i=0}^{2^{m}-1}$ and $\mathcal{P}'_{m}=\{\vx'_i\}_{i=0}^{2^{m}-1}$ accordingly to the multiplicative approach detailed in \cite{GJAHMP}.  Step $5$ corresponds to the recursive scheme of this approach. The choice of $m_0$ must be large enough to insure that each integrand of $\vI$ (Section \ref{sec:3.2}) is in $\cc$ (see  (\ref{conecond})).

With this construction, $\mathcal{P}_{m}$ and $\mathcal{P}'_{m}$ correspond to the first $2^{m}$ points of two Sobol' sequences. Furthermore, they possess the structure of two replicated designs of order $1$ (Definition \ref{rep.designs}).
\bigskip
...
In both case we test if the respective error bounds $\varepsilon_{\underline{S}_u}$ and $\varepsilon_{\overline{S}_u}$ are lower than the tolerance $\epsilon$. For Variant $A$ the stopping criterion writes: $ \forall u \in \mathcal{D}, \varepsilon_{\underline{S}_u} \leq \epsilon \text{ and } \varepsilon_{\overline{S}_u} \leq \epsilon$,
...
If Variant $B$ is selected the cost of our algorithm equals $2 \times 2^{m^{\star}}$. This cost corresponds to the one of the replication procedure with $n=2^{m^{\star}}$.
...
We discuss below the potential improvement brought by the use of ``Correlation 2" in Variant $A$. The idea is to replace the current estimator (\ref{first.order.est}) by  ``Correlation 2" for each small first-order index. 
...
\item[$\bullet$] likewise, for $j\in \mathcal{D}\setminus{\Gamma}$, $ 2^{m_{u_j}}$ is the number of evaluations $f\left(\vx_{i,j}:{\vx'}_{i,-j}\right)$ to estimate both $\underline{S}_{u_j}$ and  $\overline{S}_{u_j}$,
\item[$\bullet$] $2 \times 2^{m^{\star}}$ is the number of evaluations $f\left(\vx_{i}\right)$ and $f\left({\vx'}_{i}\right)$ used in the estimation of each first-order and total effect index.
\end{itemize}
...
This work is supported by the CITiES project funded by the Agence Nationale de la Recherche (grant ANR-12-\newline MONU-0020) and by the United States National Science Foundation (grant DMS-1522687). The authors thank Stephen Joe, Frances Y. Kuo and Art B. Owen for helpful answers and suggestions.   
...
%
\bibitem{GJAHMP}
Gilquin, L., Jim\'enez Rugama, L.A., Arnaud, E., Hickernell, F.J., Monod, H., Prieur, C.: Iterative construction of replicated designs based on Sobol' sequences, preprint available at \url{https://hal.inria.fr/hal-01349444} (2016)
\bibitem{HicJim}
