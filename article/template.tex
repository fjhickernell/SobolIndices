...
We assume that $f \in \mathcal{L}^2\left([0,1]^d\right)$ and denote by $\mu$ and $\sigma^2$ the mean $\int_{\cube}f(\vx)d{\vx}$ and variance $\int_{\cube}(f(\vx)-\mu)^2d{\vx}$ of $f$.
The uncertainty on $\vx$ is modeled as a uniform random variable, namely $\vx \iid \mathcal{U}\left([0,1)^d\right)$. The Hoeffding decomposition \cite{Hoeffding,Sobol'} of $f$ is:
\begin{equation}
...
The problem of interest is the evaluation of first-order and total effect Sobol' indices of order 1. The computation of the normalized indices is performed based on the following integral formulas for their numerators:
\begin{align}
\label{first.order}
...
\end{align}
while the variance of $f$ is evaluated as:
\begin{equation}
\sigma^2 = \int_{[0,1)^{d}} f(\vx)^2d{\vx} - \mu^2, \text{ with } \ \mu = \int_{[0,1)^{d}} f(\vx) d{\vx}.
\label{eq.sigma.mu}
\end{equation} 
Usually the complexity of $f$ causes the computation of integrals \eqref{first.order}, (\ref{total.effect}) and (\ref{eq.sigma.mu}) to be intractable. In such cases, one can estimate these quantities instead.
...
\label{mu.est}
\end{equation}
Then, the normalized Sobol' indices estimators are:
\begin{equation}
...
\label{sec:3} 
We start by reviewing the construction of the error bound proposed in \cite{HicJim} for the estimation of $d$-dimensional integrals. Then, we present an extension of this error bound for normalized Sobol' indices. This extension exploits the integral formula of a Sobol' index.
...
\mathcal{P}_0^\perp=\mathbb{N}_0^d\supset\dots\supset\mathcal{P}_\infty=\{\vect{0}\}.
\end{equation}
...
\end{cases}
\end{equation}
Therefore, considering the Walsh decomposition for any $f\in L^2([0,1]^d)$: \[
...
& \leq \sum_{\vk\in\mathcal{P}_m^\perp\setminus\{\vzero\}}\absolute{\hf_{\vk}}.
\end{align*}
...
\end{equation}
However, because the knowledge of the Walsh coefficients $\hf_{\kappa}$ is not assumed, we will estimate them using the precomputed function values instead, and refer to them as $\tf_{m,\kappa}$.
...
\begin{equation}
\absolute{I(f) - \frac{1}{b^m}\sum_{\vx\in\mathcal{P}_m}f(\vx)}
\le \overbrace{\frac{\tS_{\ell,m}(f)\homega(m) \wcomega(m-\ell)}{1 - \homega(m-\ell) \wcomega(m-\ell)}}^{\epsilon_I:=}, \label{errbd}
\end{equation}
...
\end{align*}
where $\vI=(I_1,I_2,I_3,I_4)$ is a vector of integral values. $\underline{S}_u(\vI)$ and $\overline{S}_u(\vI)$ are defined as functions over the vector $\vI$. If we estimate $\vI$ by $\widehat{\vI}$ with the vector of error bounds $\varepsilon_{\vI}=(\epsilon_{I_1},\epsilon_{I_2},\epsilon_{I_3},\epsilon_{I_4})$ according to \ref{sec:3.1}, we know that $\vI\in B_{\varepsilon_{\vI}}(\widehat{\vI})=[\widehat{\vI}-\varepsilon_{\vI},\widehat{\vI}+\varepsilon_{\vI} ]$. 
