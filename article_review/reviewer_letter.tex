...
First, the cost of constructing the error threshold is a computational cost and not a numerical cost. That is, it does not require extra model evaluations. This specificity is further discussed in the answer to point 2.

Secondly, we claim that the precision of our procedure should be better than those based on confidence intervals (asymptotic or bootstrap based) obtained with random sampling methods. In our approach, the construction of the error bounds makes use of the analytical expression of the Sobol' indices. At the opposite, errors bounds in confidence intervals are estimated entirely from the sampling points and are less accurate.

To back up this claim, the precision of our procedure is compared to bootstrap confidence intervals obtained from random sampling. The comparison is carried out for the wing weight example. Results show that for an equivalent number of evaluations, the precision of bootstrap confidence intervals is far less accurate than the precision of our procedure.}
