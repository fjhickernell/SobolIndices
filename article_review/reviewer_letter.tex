...
{\color{blue} The biased estimator has been replaced by the unbiased one.}
...
{\color{blue} The ``real case model'' has been replaced with a new one as suggested by the second reviewer. Nonetheless, we thank the reviewer for these corrections.} 
\end{itemize}
...
\answer{More details have been provided in Section 3.1 to guide readers follow the ideas behind the approach. Among all additions, $k_{j\ell}$ has been defined and the sums $S_m$, $\hS_{\ell,m}$, $\wcS_m$, $\tS_{\ell,m}$ have been briefly described in conjunction with the cone conditions.}
...
{\color{blue} The Sobol' g-function has been modified according to the reviewer remark. There were no drastic changes observed. As a matter of fact, the failure rates have improved for all three approaches (Variant A.a, A.b and B).} 
