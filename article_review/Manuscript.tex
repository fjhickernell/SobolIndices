...
\end{align*}
\comment{Among these sums, one should notice that $\hS_{0,m}(f)$ corresponds to the error bound appearing in \eqref{mapped_error_bound}. Thus, $\wcS_m(f)$ is an infinite sum of Walsh coefficients that contain the error bound coefficients. To have a control on the decay rate of the coefficients, we also define the finite sum of true coefficients $S_m(f)$. Note that from these sums, we can only observe $\tS_{\ell,m}(f)$.  }
...
\end{multline}
for a fixed $\ell_* \in \mathbb{N}$, $\homega$ and $\wcomega$ two non-negative valued functions with $\lim_{m \to \infty} \wcomega(m) = 0$. \comment{The decay rate assumptions on the Walsh coefficients are encoded in $\cc$ and controlled by $\homega$ and $\wcomega$. First, we assume that $\wcS_m(f)$ bounds $\hS_{\ell,m}(f)$ in some sense, in particular it bounds $\hS_{0,m}(f)$. This means that we can use $\wcS_m(f)$ as a good indicator for the error bound. Then, we also need $S_{\ell}(f)$ to bound $\wcS_m(f)$ which helps transforming the infinite sum into a finite sum. In this step, we are assuming that if $S_{\ell}(f)$ is small enough, we expect the high frequency coefficients, i.e. $\wcS_m(f)$, to also be small. By defining $r=m-\ell$, and assuming that $\homega(r)\wcomega(r)<1$ with $r\in\mathbb{N}$, one may see $r$ as a distance between $S_{\ell}(f)$ and $\wcS_m(f)$: larger values of $r$ will imply a smaller error bound but more dependence on smaller Walsh coefficients. Finally, we can write the error bound in terms of $\tS_{\ell,m}(f)$ instead of $\wcS_m(f)$ by using \eqref{discrite_true_relationship}.} In \cite[Sec. 4.2]{HicJim} it is shown that for any $f\in\cc$,
\begin{equation}
\absolute{I - \frac{1}{b^m}\sum_{\vx\in\mathcal{P}_m}f(\vx)}
\le \tS_{\ell,m}(f)\underbrace{\frac{\homega(m) \wcomega(r)}{1 - \homega(r) \wcomega(r)}}_{\mathfrak{C}(m)}=\epsilon_{\widehat{I}}, \label{errbd}
\end{equation}
...
\end{itemize}
{\color{purple}(Lolo) We assume that the effective dimension of the numerator of $\underline{S}_u$ is higher than the one of the numerator of $\ov{$S$}_u$. If this assumption holds, we expect to observe case $2)$ more often than case $1)$ and the sign of \eqref{cost.comparison} should be negative or equal to $0$. The validity of this assumption is tested on the numerical examples presented in Section \ref{sec:5}.
...
{\color{purple} (Lolo) To verify the validity of our  assumption saying that the sign of \eqref{cost.comparison} is negative, the sum of the $c_j$ is evaluated for each repetition and barplots of its values are drawn.}
...
{\color{purple} (Lolo) (REFAIRE BARPLOT MOINS HAUT) 
\begin{figure}[!ht]
\caption{Barplots of the $100$ values taken by the sum of the $c_j$. The distribution is bimodal with $60\%$ values equal to $0$.}
\vspace*{0.2cm}
\centering
\includegraphics[width=0.45\textwidth]{sobolg_cj.eps}
\label{cj.gfunc}
\end{figure}
Figure \ref{cj.gfunc} shows barplots of the $100$ values of the sum of the $c_j$. The distribution is bimodal with around $60\%$ of the values equal to $0$ and $40\%$ equal to $512$. As a result, the assumption is validated $60\%$ of the times and Variant A.b does not cost more (or slightly more) than Variant A.a.}
\bigskip
