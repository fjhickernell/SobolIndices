...
\comment{We assumed that evaluating our model is expensive. Thus, we will focus our cost study on the number of function evaluations needed to estimate the indices. The function evaluations used to estimate the indices are the same as the ones used to compute the discrete Walsh coefficients. The only computation cost that could be expensive is the fast transform algorithm. However, on a regular laptop computing the discrete coefficients of $2^{20}$ points takes around $0.5$ seconds while evaluating these points at an average evaluation time of $1$ microsecond, takes a total of around $1$ second.}
