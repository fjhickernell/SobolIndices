...
%assumptions
%
%   [q,out_param] = CUBSOBOL_SI_G(f,hyperbox,u) estimates the integral of f
...
%% Main algorithm
sobstr=sobolset(2*out_param.d-1); %generate a Sobol' sequence
sobstr=scramble(sobstr,'MatousekAffineOwen'); %scramble it
Stilde=zeros(out_param.mmax-out_param.mmin+1,4); %initialize sum of DFWT terms
CStilde_low = -inf(out_param.mmax-l_star+1,4); %initialize various sums of DFWT terms for necessary conditions
CStilde_up = inf(out_param.mmax-l_star+1,4); %initialize various sums of DFWT terms for necessary conditions
errest=zeros(out_param.mmax-out_param.mmin+1,4); %initialize error estimates
...
xpts=sobstr(1:n0,1:2*out_param.d+1); %grab Sobol' points
...
function [f,hyperbox, out_param.u, out_param] = cubSobol_g_param(r_lag,varargin)
...
if numel(varargin)<3
    ...
    if ~gail.isfcn(f)
        ...
        if ~isnumeric(hyperbox) || ~(size(hyperbox,1)==2) || ~(size(hyperbox,2)<1111)
            ...
            u = varargin{3};
            if ~isinteger(u) || abs(u) > size(hyperbox,2) || u == 0
                warning('GAIL:cubSobol_g:u_error1',...
